...
export default function App() {
  ...
  const [activePage, setActivePage] = useState('dashboard'); // 'dashboard', 'control-room', 'todos'
  const [activeTab, setActiveTab] = useState('combined');   // Quick Filter Metric Slots
  const [loading, setLoading] = useState(false);

  // Discrete Segregation State Fields
  ...
  // Standalone Todo Engine State (Stored locally)
  const [todos, setTodos] = useState(() => JSON.parse(localStorage.getItem('grid_todos') || '[]'));
  const [newTodo, setNewTodo] = useState('');

  // Event Input Form Storage
  const [formTitle, setFormTitle] = useState('');
  const [formStart, setFormStart] = useState('');
  const [formDescription, setFormDescription] = useState('');
  const [formCalendar, setFormCalendar] = useState('school');
  ...
  const fetchAllFeeds = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Dynamic Allocation to Specific Category Containers
        setZoeEvents(rawFeedData.filter(e => e.domain === 'zoe'));
        setWorkEvents(rawFeedData.filter(e => e.domain === 'work'));
        setKidsLogs(rawFeedData.filter(e => e.domain === 'kids-logs'));
        setLiamLifeEvents(rawFeedData.filter(e => e.domain === 'liam-life'));
        
        // 🚨 CRITICAL FIX: Explicit absolute fallback routing for the public Abington stream
  ...
  const handleDeleteEvent = async (id) => {
    if (!window.confirm("Purge event record irreversibly from server databanks?")) return;
    try {
  ...
  // Determine Active Feed Segment Matrix array mapping
  let displayEvents = [];
  ...
  // Real-time Horizon Alerts (Next 72 Hours)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans antialiased pb-24 md:pb-6">
      ...
      <header className="bg-slate-900/60 backdrop-blur-xl border-b border-slate-800/60 sticky top-0 z-50 px-4 py-3.5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] text-emerald-400 font-mono tracking-widest uppercase flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Database Online
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleDownloadPDF} title="Download system verification log file" className="p-2 bg-slate-950 border border-slate-800 text-slate-400 hover:text-indigo-400 rounded-xl transition-all shadow-sm flex items-center gap-1 text-xs font-medium">
      ...
      {/* CORE FRAME ROUTING ROUTINE */}
      <main className="max-w-7xl mx-auto w-full p-4 md:p-6 flex-1">
        
        {activePage === 'dashboard' && (
          <div className="space-y-6">
            
            {/* INTERFACE QUICK HORIZONTAL ACCORDION DATA BLOCKS */}
            <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
              {[
                ...
                { id: 'kids-logs', name: 'Child Matrix logs', count: kidsLogs.length, color: 'border-orange-500/30 text-orange-400' }
              ].map(stat => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* PRIMARY TIMELINE DATA BLOCK (Spans Left side) */}
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 md:p-6 shadow-xl flex flex-col min-h-[550px]">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-base font-bold tracking-tight uppercase font-mono">{activeTab.replace('-', ' ')} Live Logs Feed</h2>
                    </div>
                    <button onClick={fetchAllFeeds} className="text-xs font-mono text-indigo-400 hover:underline">Force Feed Sync</button>
                  </div>

                  {loading ? (
                    <div className="flex-1 flex flex-col items-center justify-center gap-2">
                      <div className="w-6 h-6 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>
                      <p className="text-xs font-mono text-slate-500">Processing live cache logs...</p>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-950/30 border border-dashed border-slate-800 rounded-2xl">
                      ...
                      <p className="text-xs font-medium text-slate-400">No logs or active events present on this route filter configuration.</p>
                    </div>
                  ) : (
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-lg space-y-3">
                  ...
                  {urgentHorizonEvents.length === 0 ? (
                    <p className="text-[11px] text-slate-500 italic bg-slate-950 p-3 rounded-xl border border-slate-800/40">Horizon matrix lines stable.</p>
                  ) : (
                ...
                {/* SIDEBAR ACCORDION TASK TRACKER */}
                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-lg space-y-3">
        ...
        {/* CORE ROOM VIEW: ENTRY CREATION GATEWAY */}
        {activePage === 'control-room' && (
          <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800/80 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
            <div className="border-b border-slate-800 pb-3">
              <h2 className="text-xl font-black tracking-tight flex items-center gap-2"><Plus className="w-5 h-5 text-indigo-400" /> Log Core Registry Entry</h2>
              <p className="text-xs text-slate-400 mt-0.5">Inject verified track items straight through custom internal server streams.</p>
            </div>
            <form onSubmit={handleCreateEvent} className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Event Log Label Header</label>
                <input type="text" value={formTitle} onChange={e => setFormTitle(e.target.value)} placeholder="e.g. Incidents Tracker Metric Point" required className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Target Domain Target Allocation</label>
                  <select value={formCalendar} onChange={e => setFormCalendar(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none">
                    ...
                    <option value="work">Corporate Operations Feed</option>
                  </select>
                </div>
                <div>
        ...
        {/* FULL PAGE STANDALONE CHECKLIST SCREEN HUB */}
        {activePage === 'todos' && (
      ...
      {/* FIXED FOOTER NAV BAR SYSTEM FOR TOUCHSCREEN PACKETS */}
